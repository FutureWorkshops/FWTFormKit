//
//  FWTCellsConstants.h
//  FWTCalculator
//
//  Created by Yevgeniy Prokoshev on 19/11/2015.
//  Copyright © 2015 Yevgeniy Prokoshev. All rights reserved.
//

#import <Foundation/Foundation.h>

FOUNDATION_EXPORT NSString * const FWTNameAndLabelRow;
FOUNDATION_EXPORT NSString * const FWTNameAndTextRow;
FOUNDATION_EXPORT NSString * const FWTNameAndValueRow;
FOUNDATION_EXPORT NSString * const FWTPickerRow;
FOUNDATION_EXPORT NSString * const FWTValuePickerRow;
FOUNDATION_EXPORT NSString * const FWTNameAndSwitchRow;
